...
<blockquote>configure --disable-all --enable-cli --with-curl --enable-calendar --enable-ctype --enable-fileinfo --enable-filter --with-gd --enable-hash --with-iconv --enable-json --enable-mbstring --with-mcrypt --with-mysqli --with-openssl --enable-pdo --with-pdo-mysql --enable-session --enable-zlib --enable-apache2-4handler</blockquote>
8. To build PHP, run:
